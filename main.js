...
// ════════════════════════════════════════
//  ── RE-EXPORT MODULE ──
//  MPA Tariff — Transhipment / Re-Shipment & Payable Charges
//  Multiple Bill of Entries, each with multiple CLD lots.
// ════════════════════════════════════════
  ...
  container.innerHTML = reexportBEs
    .map((be, bi) => {
            ...
            <div class="pbs-fields pbs-fields-3col">
              ...
              <div class="fg">
                ...
                <div class="in-unit-wrap">
                  <input type="number" id="re-be-ton-${bi}" class="cyan-glow" placeholder="0" min="0" step="0.01"
                    value="${ton ? ton : ""}"
                    oninput="reexportBEs[${bi}].clds[0].ton=Math.max(0,Number.parseFloat(this.value)||0); reexportRefresh();" />
          ...
          return `<div class="re-cld-row" id="re-cld-row-${bi}-${ci}">
            <div class="r2">
              ...
              <div class="fg">
                ...
                <div class="in-unit-wrap">
                  <input type="number" id="re-cld-ton-${bi}-${ci}" class="cyan-glow" placeholder="0" min="0" step="0.01"
                    value="${cld.ton ? cld.ton : ""}"
                    oninput="reexportBEs[${bi}].clds[${ci}].ton=Math.max(0,Number.parseFloat(this.value)||0); reexportRefresh();" />
                  <span class="in-unit">ton</span>
                </div>
              </div>
            </div>
      ...
      const removalField = `<div class="fg">
            ...
            <div class="in-unit-wrap">
              <input type="number" id="re-be-removal-${bi}" class="cyan-glow" placeholder="0" min="0" step="0.01"
                value="${be.removalTon ? be.removalTon : ""}"
                oninput="reexportBEs[${bi}].removalTon=Math.max(0,Number.parseFloat(this.value)||0); reexportRefresh();" />
              <span class="in-unit">ton</span>
            </div>
          </div>`;
  ...
  const beResults = reexportBEs
    .map((be, idx) => {
      const clds = be.clds
        ...
        .map((c) => ({ date: pd(c.date), dateStr: c.date, ton: Math.max(0, Number.parseFloat(c.ton) || 0) }));
      ...
      const removalTon = !isOverside ? Math.max(0, Number.parseFloat(be.removalTon) || 0) : 0;
...
function collectReexportErrors() {
  ...
  reexportBEs.forEach((be, bi) => {
    be.clds.forEach((cld, ci) => {
      const ton = Math.max(0, Number.parseFloat(cld.ton) || 0);
